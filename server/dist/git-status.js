 ...
 */
export async function computeGitDiffStat(cwd) {
    ...
    let diffOutput = "";
    try {
        const { stdout } = await execFile("git", ["diff", "--numstat", "HEAD"], { cwd, timeout: GIT_TIMEOUT_MS, maxBuffer: GIT_MAX_BUFFER });
        diffOutput = stdout;
    }
    catch {
        // HEAD might not exist (empty repo). Try --cached only.
        try {
            const { stdout } = await execFile("git", ["diff", "--cached", "--numstat"], { cwd, timeout: GIT_TIMEOUT_MS, maxBuffer: GIT_MAX_BUFFER });
            diffOutput = stdout;
        }
        catch {
            // No diff available
    ...
    // 2. Mark staged files
    try {
        const { stdout } = await execFile("git", ["diff", "--cached", "--name-only"], { cwd, timeout: GIT_TIMEOUT_MS, maxBuffer: GIT_MAX_BUFFER });
    ...
    // 3. Untracked files
    try {
        const { stdout } = await execFile("git", ["ls-files", "--others", "--exclude-standard"], { cwd, timeout: GIT_TIMEOUT_MS, maxBuffer: GIT_MAX_BUFFER });
        for (const line of stdout.split("\n")) {
            const path = line.trim();
            if (!path)
                continue;
